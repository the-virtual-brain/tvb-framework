...
<xsl:template name="summary">
	...
	<table class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
		...
		<tr valign="top">
			...
			<xsl:variable name="allMinTime" select="min(/testResults/*/@t)"/>
			<xsl:variable name="allMaxTime" select="max(/testResults/*/@t)"/>
    ...
    <table class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
        <tr>
            <th>Thread Group</th>
            <th>No. of threads</th>
            <th>Avg. time (milliseconds)</th>
            <th>Success</th>
        </tr>
        <xsl:for-each-group select="/testResults/*" group-by="substring-before(@tn,'-')">
            <tr>
                <td><xsl:value-of select="substring-before(@tn,'-')" /></td>
                <td><xsl:value-of select="count(distinct-values(current-group()/@tn))" /></td>
                <td align="right"><xsl:value-of select="round(avg(current-group()/@t)*100) div 100" /></td>
...
<xsl:template name="pagelist">
	...
	<table class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
		...
		<xsl:for-each select="/testResults/*[not(@lb = preceding::*/@lb)]">
			...
			<xsl:variable name="minTime" select="min(../*[@lb = current()/@lb]/@t)"/>
			<xsl:variable name="maxTime" select="max(../*[@lb = current()/@lb]/@t)"/>
			<tr valign="top">
				<xsl:attribute name="class">
					<xsl:choose>
				...
				<td align="center">
				   <a href="">
                    ...
                    <a href="">
				      <xsl:attribute name="href"><xsl:text/>javascript:change('page_detailsG_<xsl:value-of select="position()" />')</xsl:attribute>
				      <img src="expand.jpg" alt="expand/collapse"><xsl:attribute name="id"><xsl:text/>page_detailsG_<xsl:value-of select="position()" />_image</xsl:attribute></img>				      
				   </a>
                </td>
			</tr>
            ...
            <tr class="page_details">
                <xsl:attribute name="id"><xsl:text />page_detailsG_<xsl:value-of select="position()" /></xsl:attribute>
                <td colspan="8" bgcolor="#FF0000">
                    <div align="center">
                        <b>Group details for Page "<xsl:value-of select="$label" />"</b>
                        <table bordercolor="#000000" border="1" cellpadding="0" cellspacing="0" width="95%">
                            <tr>
                                <th>Thread Group</th>
                                <th>No. of threads</th>
                            ...
                            <xsl:for-each-group select="../*[@lb = $label and @tn != $label]" group-by ="substring-before(@tn,'-')">
                                    <tr>
                                        <td><xsl:value-of select="substring-before(@tn,'-')" /></td>
                                        <td><xsl:value-of select="count(distinct-values(current-group()/@tn))" /></td>
                                        <td align="right"><xsl:value-of select="avg(current-group()/@t)" /></td>
                                        <td align="right"><xsl:value-of select="sum(current-group()/@by)" /></td>
                                        <td align="center"><xsl:value-of select="@s" /></td>
                                    </tr>
                            </xsl:for-each-group>
                        </table>
